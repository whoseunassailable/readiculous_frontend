...
class _GenrePreferencesPageState extends ConsumerState<GenrePreferencesPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: StylishAppBar(title: 'Genre Preferences', homepage: false),
      body: Container(
  ...
  Future<void> _save(String userId, Set<String> currentGenreIds) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Genre preferences updated.')),
      );
    } catch (_) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: selected ? color : const Color(0xFFFFFDF3),
